...
class SymbolMapper:
    ...
    def batch_map_symbols(self, capital_epics: List[str], validate: bool = True) -> Dict[str, Optional[str]]:
        """
    ...
    def get_mapping_stats(self) -> Dict[str, int]:
        ...
        validated_count = sum(1 for epic, symbol in self.mapping.items() 
...
def create_initial_mapping_from_capital_data(csv_file: str, output_file: str = "data/symbol_mapping.json"):
    """
    ...
    for i in range(0, len(epics), batch_size):
        ...
        logger.info(f"Processing batch {i//batch_size + 1}/{(len(epics)-1)//batch_size + 1}")
    ...
    logger.info(f"Mapping complete: {successful}/{len(epics)} symbols mapped successfully")
