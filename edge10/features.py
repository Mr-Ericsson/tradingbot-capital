...
def calculate_day_range_pct(open_price: float, high_price: float, low_price: float) -> float:
    """
...
def calculate_features(
    ...
    try:
        ...
        features.update({
            ...
            'AdjClose': round(safe_float(latest_day.get('Adj Close', latest_day['Close'])), 2),
        ...
        features.update({
            'AvgVol10': round(avg_vol_10, 0),
            'RelVol10': round(rel_vol_10, 4)
        })
        ...
        features.update({
            ...
            'SectorStrength': round(sector_strength, 4)
        })
        ...
        # SEC filings
        start_dt = datetime.combine(market_date - timedelta(days=1), datetime.min.time())
        ...
        logger.debug(f"Features calculated for {ticker}: ATR={atr_pct:.4f}, RelVol={rel_vol_10:.2f}")
