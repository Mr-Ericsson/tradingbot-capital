#!/usr/bin/env python3
"""
EdgeScore calculation for EDGE-10 system

Implements the sophisticated scoring algorithm with:
- 30% Price Momentum (DayStrength)
- 30% Volume Power (RelVol10) 
- 20% Catalyst Power (News/Earnings/SEC)
- 10% Market Strength (IndexBias + SectorStrength)
- 10% Volatility Fit (ATRpct sweet spot)
...
def calculate_catalyst_score(news_flag: int, earnings_flag: int, sec_flag: int) -> float:
    """
...
def calculate_market_strength_score(index_bias: int, sector_strength_rank: float) -> float:
    """
...
def calculate_edge_score(features_list: List[Dict]) -> List[Dict]:
    ...
    try:
        ...
        for features in features_list:
            ...
            sector_strength_values.append(safe_float(features.get('SectorStrength', 0.0)))
        ...
        sector_strength_ranks = calculate_rank_score(sector_strength_values, ascending=False)
        ...
        for i, features in enumerate(features_list):
            try:
                ...
                logger.debug(f"{features['Ticker']}: EdgeScore={final_edge_score:.1f}, Reason={pick_reason}")
                
            except Exception as e:
                logger.error(f"Error calculating EdgeScore for {features.get('Ticker', 'unknown')}: {e}")
                features.update({
                    'EdgeScore': 0.0,
        ...
        logger.info(f"EdgeScore calculation completed. Top score: {features_list[0].get('EdgeScore', 0):.1f}")
