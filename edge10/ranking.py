...
def apply_hard_filters(
    df: pd.DataFrame, min_relvol: float = 1.30, relax_relvol: bool = True
) -> pd.DataFrame:
    """
    ...
    mask_instruments = ~df.apply(
        lambda row: is_leveraged_etf(row.get("Ticker", ""), row.get("Name", "")), axis=1
    )
    df = df[mask_instruments]
    logger.info(
        f"  After instrument filter: {len(df)} ({initial_count - len(df)} removed)"
    )
    ...
    df = df[mask_price]
    logger.info(
        f"  After price filter: {len(df)} ({mask_instruments.sum() - len(df)} removed)"
    )
    ...
    logger.info(
        f"Final filter result: {len(df)} candidates (from {initial_count} initial)"
    )
    return df


def select_top10_with_sector_cap(
    df: pd.DataFrame, max_per_sector: int = 4
) -> pd.DataFrame:
    """
    ...
    """
    logger.info(
        f"Selecting TOP-10 from {len(df)} candidates with sector cap {max_per_sector}"
    )
    ...
    if len(selected) < 10:
        ...
        remaining_indices = [
            idx for idx in df.index if idx not in [row.name for row in selected]
        ]
...
def save_results(df: pd.DataFrame, output_dir: Path, file_prefix: str) -> str:
    ...
    for col in ["Volume", "AvgVol10"]:
        if col in df_formatted.columns:
            df_formatted[col] = df_formatted[col].apply(
                lambda x: f"{safe_float(x):.0f}"
            )

    # EdgeScore: 1 decimal
    if "EdgeScore" in df_formatted.columns:
        df_formatted["EdgeScore"] = df_formatted["EdgeScore"].apply(
...
def rank_universe(
    ...
    for idx, row in capital_df.iterrows():
        ...
        try:
            logger.debug(
                f"Processing {ticker} ({processed_count + 1}/{len(capital_df)})"
            )
            ...
            features.update({"Epic": epic, "Name": name, "SpreadPct": spread_pct})
            ...
            if processed_count % 50 == 0:
                logger.info(
                    f"Processed {processed_count}/{len(capital_df)} instruments"
                )
    ...
    logger.info(
        f"Feature calculation complete: {len(all_features)} instruments processed"
    )
    ...
    for idx, row in top_10_df.iterrows():
        logger.info(
            f"  {idx+1:2d}. {row['Ticker']:5s} EdgeScore: {row['EdgeScore']:5.1f} - {row.get('PickReason', 'N/A')}"
        )
