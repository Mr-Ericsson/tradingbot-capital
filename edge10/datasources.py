...
def get_history_yahoo(
    ticker: str, period: str = "400d", use_symbol_mapper: bool = True
    ...
    try:
        ...
        if use_symbol_mapper:
            mapper = SymbolMapper()
            mapped_symbol = mapper.map_symbol(
                ticker, validate=False
...
def get_index_bias(index_ticker: str, target_date: date) -> int:
    ...
    try:
        ...
        logger.debug(
            f"{index_ticker} bias on {latest_day['Date']}: {bias} (O:{open_price:.2f} C:{close_price:.2f})"
        )
...
def get_finnhub_news(
    ticker: str, start_dt: datetime, end_dt: datetime, api_key: Optional[str] = None
) -> List[Dict]:
    """
    ...
    try:
        ...
        params = {"symbol": ticker, "from": start_ts, "to": end_ts, "token": api_key}
        ...
        try:
            ...
            if calendar is not None and not calendar.empty:
                ...
                for earnings_date in calendar.index:
                    ...
                    if date_diff <= 1:
                        logger.debug(
                            f"Earnings flag for {ticker}: {earnings_date} near {target_date}"
                        )
                        return True
        except:
            pass
