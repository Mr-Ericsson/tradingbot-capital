...
def main():
    ...
    # Välj vad du vill handla (ex. TSLA eller AMC)
    ...
    if not position_deal_id or str(status).upper() != "ACCEPTED":

        print("[stop] order ej accepterad – avbryter SL/TP.")
        return

    # 3) Hämta entry (offer) + tick-decimals
    m = session.get(f"{BASE}/api/v1/markets/{epic}", timeout=10).json()
    entry = float(m["snapshot"]["offer"])
    decimals = _get_tick_decimals(session, BASE, epic)

    # 4) Räkna SL/TP = ±3% från entry och runda
    sl = round(entry * 0.97, decimals)
    tp = round(entry * 1.03, decimals)
    print(f"[levels] entry={entry} -> SL={sl} TP={tp} (decimals={decimals})")

    # 5) Sätt SL/TP efter fill
    attach_sl_tp(position_deal_id, sl, tp)
