...
DEFAULT_ALLOWED_TYPES = {"SHARES", "INDICES", "COMMODITIES", "ETF", "ETFS"}
CONNECT_TIMEOUT = 5
READ_TIMEOUT = 30
POOL_SIZE = 32
SCAN_OUTPUT_PATH = "scan_tradeable_current.csv"
...
def row_from_market(m, max_spread_frac, allowed_types):
    ...
    if spread / offer > max_spread_frac:
        return None
    return {
        "epic": m.get("epic"),
        "bid": float(bid),
        ...
        "spread_pct": round(spread / offer * 100, 4),
...
def main():
    ...
    ap.add_argument("--spread", type=float, default=DEFAULT_MAX_SPREAD_FRAC)
    ap.add_argument("--types", type=str, default=",".join(DEFAULT_ALLOWED_TYPES))
    ...
    rows = []
    for m in markets:
        r = row_from_market(m, args.spread, set(args.types.split(",")))
        if r:
            rows.append(r)
    ...
    for r in sorted(enriched, key=lambda x: x["spread_pct"])[:20]:
        print(
            f"{r['epic']:<12} {r['bid']:>12.6f} {r['offer']:>12.6f} {r['spread']:>10.6f} {r['spread_pct']:>7.3f}% {(r['name'] or '')[:40]}"
        )
